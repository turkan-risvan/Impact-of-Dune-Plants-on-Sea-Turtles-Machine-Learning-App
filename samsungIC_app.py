...
#base="light"
#primaryColor="#52c17b"
#backgroundColor="#d5e8c9"
#secondaryBackgroundColor="#a5c199"

#anaconda promptta:
#cd C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone
#streamlit run samsungIC_app.py
...
#st.title("Impact of Dune Plants on Loggerhead Turtles")
#st.text("xxxxxx")

# Yan yana düzenlemek için Streamlit'in beta_columns özelliğini kullanın
# Diğer Streamlit kodunuz

def get_raw_data():
    """
    This function returns a pandas DataFrame with the raw data.
    """
    raw_df = pd.read_csv(r'C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\dryad_dataset\ReddingRootsCaseStudy22_csv.csv')
    raw_df = raw_df[0:93]
...
def get_cleaned_data():
    ...
    """
    clean_data = pd.read_csv(r'C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\cleaned_df.csv')
    return clean_data
...
if condition == 'Introduction':
    ...
    with col1:
        st.image("logo_turtle.png", width=140)  # Logo dosyanızın doğru yolunu belirtin
    with col2:
    ...
    # Görseli ekleme
    image1 = Image.open(r'C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\seaturtle_img.jpg')
    ...
    image2 = Image.open(r'C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\piechart.png')
    ...
    image2 = Image.open(r'C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\sdg14.png')
    width, height = image2.size
...
########### outlierları görme: ##############
    ...
    if st.checkbox("Correlation Matrix"):
        # Heatmap grafiğini oluşturalım
        plt.figure(figsize=(10, 8))  # Genişlik: 10, Yükseklik: 8
        sns.heatmap(df_raw[num_cols].corr(), annot=True, cmap='coolwarm',
                    cbar_kws={'label': 'Korelasyon'})  # Renk skalası etiketi ekliyoruz
        plt.title('Korelasyon Matrisi (Heatmap)', fontsize=15)
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)

        # Grafiği Streamlit uygulamasında gösterelim
        st.pyplot()
       ...
       'MultiVegNum_4']
        ...
        if st.button('Predict'):
            ...
            if model_option == "Linear Regression":
                model = load_model(r"C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\IC_lin_reg_model.pkl")
                prediction = model.predict(last_row)
                st.write('## Result')
                st.success(f"Hatching Success Prediction: {prediction.item()}")
            elif model_option == "Decision Tree":
                model = load_model(r"C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\IC_cart_model.pkl")
                prediction = model.predict(last_row)
                st.write('## Result:')
                st.success(f"Hatching Success Prediction: {prediction.item()}")
            elif model_option == "Random Forest":
                model = load_model(r"C:\Users\kilic\PycharmProjects\cerenPyProje\samsung_IC_capstone\IC_rf_model.pkl")
                prediction = model.predict(last_row)
                st.write('## Result:')
                st.success(f"Hatching Success Prediction: {prediction.item()}")
